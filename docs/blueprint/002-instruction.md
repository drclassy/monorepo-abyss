...
> Historical note: instruksi ini dipertahankan sebagai blueprint implementasi, tetapi path package di bawah sudah dinormalisasi ke taxonomy repo aktif per Mei 2026.
...
mkdir -p packages/shared/sentra-ui packages/sentra/sentra-nada packages/clinical/clinical-references packages/platform/langflow-client packages/platform/database packages/sentra/sentra-sandi packages/library/iskandar-gatekeeper packages/sentra/sentra-cermin packages/shared/shared-types
...
Buat file `docs/templates/001-handoff.md` sebagai blueprint standar untuk setiap tugas:
...
- **Compliance:** Semua mutasi data klinis harus divalidasi via `packages/sentra/sentra-sandi`.
...
1. Buat `packages/tooling/config-typescript/package.json` dan `tsconfig.json` (base config).
2. Buat `packages/tooling/config-eslint/package.json` dan `.eslintrc.js` (strict rules).
3. Hubungkan paket lain agar meng-extend konfigurasi ini.

####

Langkah 2: Database Layer (`packages/platform/database`)
...
Langkah 4: Clinical Engine & Shared Decision Surface (`packages/sentra/sentra-nada`)
...
Langkah 5: RAGOps & Vector Store (`packages/sentra/sentra-cermin`)
...
3. Integrasikan dengan `packages/platform/database` agar ID database SQL sinkron dengan ID Vektor.

####

Langkah 6: Langflow Client SDK (`packages/platform/langflow-client`)
...
3. Tambahkan logging otomatis ke `packages/platform/database` setiap kali sebuah flow dijalankan.

####

Langkah 7: Shared Types (`packages/shared/shared-types`)
...
2. **Abyss Bridge:** Buat komponen kustom yang memungkinkan Langflow memanggil fungsi dari `packages/sentra/sentra-sandi` atau melakukan query langsung ke `packages/platform/database`.
3. **Audit Node:** Buat node khusus yang secara otomatis mengirim log eksekusi ke `packages/platform/database` (Audit Trail).
...
2. **Comparison Engine:** Gunakan `packages/sentra/sentra-nada` atau evaluator domain aktif untuk membandingkan output Primary vs Shadow dan memberikan skor kualitas.
...
3. **Strict Integration:**
    - Gunakan `packages/sentra/sentra-sandi` untuk setiap input/output data medis.
    - Gunakan `packages/platform/database` untuk persistensi data (PostgreSQL/Prisma).
...
2. **Visualizer:** Buat dashboard untuk:
    ...
    - Menampilkan statistik eksekusi AI dari `packages/platform/database` (Audit Log).
...
1. **Scaffold:** Jika lane incubator diaktifkan kembali, buat aplikasi minimalis `apps/incubator/edge-ai-prototype` untuk menguji **SLM (Small Language Models)** seperti Phi-3 atau Llama 3 (Quantized).
...
1. Implementasikan middleware global di setiap aplikasi backend (`apps/healthcare`, `apps/orchestrator`) yang secara otomatis mencatat setiap request/response ke dalam tabel `AuditLog` di `packages/platform/database`.
...
1. **Provisioning:** Buat module Terraform dasar untuk:
    - Database PostgreSQL (untuk `packages/platform/database`).
    - Vector Database (untuk `packages/sentra/sentra-cermin`).
