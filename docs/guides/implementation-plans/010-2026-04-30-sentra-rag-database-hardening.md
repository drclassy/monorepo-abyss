...
**Architecture:** Keep `@the-abyss/document-ingestion` as the canonical PDF parsing and normalization front door. Make `@sentra/pustaka` consume canonical artifacts instead of duplicating parsing logic, then harden retrieval and database writes through a clearer repository abstraction, configurable model/runtime defaults, and stronger evaluation coverage. Preserve the existing local-first / PHI-safe boundary and avoid moving healthcare data into `packages/platform/database`.

**Tech Stack:** TypeScript, Vitest, PostgreSQL + pgvector, Ollama, LiteParse, `@the-abyss/document-ingestion`, `@sentra/cermin`, existing Sentra registry and evaluation artifacts.
...
1. **Preferred path:** consolidate ingestion around `document-ingestion`, then harden retrieval and DB persistence in `sentra-pustaka`.
...
**Files:**
- Modify: `packages/platform/document-ingestion/src/ingest-document.ts`
- Modify: `packages/platform/document-ingestion/src/types.ts`
- Modify: `packages/platform/document-ingestion/src/index.ts`
- Test: `packages/platform/document-ingestion/tests/ingest-document.test.ts`
...
```bash
git add packages/platform/document-ingestion/src packages/platform/document-ingestion/tests
...
**Files:**
- Modify: `packages/sentra/sentra-pustaka/src/ingestion/pipeline.ts`
- Modify: `packages/sentra/sentra-pustaka/src/ingestion/pdf-batch-runner.ts`
- Modify: `packages/sentra/sentra-pustaka/src/ingestion/artifact-writer.ts`
- Modify: `packages/sentra/sentra-pustaka/src/ingestion/chunker.ts`
- Modify: `packages/sentra/sentra-pustaka/src/cli/ingest-pdf.ts`
- Test: `packages/sentra/sentra-pustaka/tests/pdf-batch-runner.test.ts`
- Test: `packages/sentra/sentra-pustaka/tests/artifact-writer.test.ts`
...
Run: `pnpm --filter @sentra/pustaka test -- --runInBand`
Expected: the ingestion test fails until `sentra-pustaka` consumes canonical output.
...
Run: `pnpm --filter @sentra/pustaka test`
Expected: PASS

- [ ] **Step 5: Commit**

```bash
git add packages/sentra/sentra-pustaka/src/ingestion packages/sentra/sentra-pustaka/src/cli packages/sentra/sentra-pustaka/tests
git commit -m "feat(sentra-pustaka): consume canonical PDF artifacts"
```
...
**Files:**
- Modify: `packages/sentra/sentra-cermin/src/store.ts`
- Modify: `packages/sentra/sentra-cermin/src/index.ts`
- Modify: `packages/sentra/sentra-pustaka/src/storage/pgvector.store.ts`
- Modify: `packages/sentra/sentra-pustaka/src/embedding/pg-adapter.ts`
- Test: `packages/sentra/sentra-cermin/src/__tests__/store.test.ts`
- Test: `packages/sentra/sentra-pustaka/tests/approved-embedding.pipeline.test.ts`
...
Run: `pnpm --filter @sentra/cermin test`
Expected: fail until the repository abstraction is explicit and covered.

- [ ] **Step 3: Write minimal implementation**

```ts
// Centralize upsert/query logic in sentra-cermin.
...
```bash
git add packages/sentra/sentra-cermin/src packages/sentra/sentra-pustaka/src/storage packages/sentra/sentra-pustaka/src/embedding
git commit -m "feat(sentra-cermin): harden pgvector repository abstraction"
```
...
**Files:**
- Modify: `packages/sentra/sentra-pustaka/src/engine.ts`
- Modify: `packages/sentra/sentra-pustaka/src/assessment/gemma.ts`
- Modify: `packages/sentra/sentra-pustaka/src/ingestion/embedder.ts`
- Modify: `packages/sentra/sentra-pustaka/src/retrieval/local.engine.ts`
- Modify: `packages/sentra/sentra-pustaka/src/retrieval/hybrid.engine.ts`
- Modify: `packages/sentra/sentra-pustaka/src/types.ts`
- Test: `packages/sentra/sentra-pustaka/tests/runtime-config.test.ts`
...
```bash
git add packages/sentra/sentra-pustaka/src
git commit -m "feat(sentra-pustaka): make runtime models configurable"
```
...
**Files:**
- Modify: `packages/sentra/sentra-pustaka/src/retrieval/*.ts`
- Modify: `packages/sentra/sentra-pustaka/src/evaluation/*.ts`
- Modify: `packages/sentra/sentra-pustaka/src/cli/eval-retrieval.ts`
- Test: `packages/sentra/sentra-pustaka/tests/quality-scorer.test.ts`
- Test: `packages/sentra/sentra-pustaka/tests/eval-pipeline.test.ts`
...
```bash
git add packages/sentra/sentra-pustaka/src/evaluation packages/sentra/sentra-pustaka/src/retrieval packages/sentra/sentra-pustaka/tests
git commit -m "feat(sentra-pustaka): improve retrieval quality controls"
```
...
**Files:**
- Modify: `packages/sentra/sentra-pustaka/tests/pipeline-safety.test.ts`
- Modify: `packages/platform/document-ingestion/tests/ingest-document.test.ts`
- Modify: `packages/sentra/sentra-cermin/src/__tests__/store.test.ts`
- Modify: `packages/sentra/sentra-pustaka/README.md`
- Modify: `packages/platform/document-ingestion/README.md`
...
```bash
git add packages/platform/document-ingestion packages/sentra/sentra-cermin packages/sentra/sentra-pustaka
git commit -m "docs(sentra-pustaka): close the database-safe RAG contract"
```
...
- `sentra-pustaka` no longer depends on raw PDF extraction as its primary path.
...
- Do not move healthcare data into `packages/platform/database`.
