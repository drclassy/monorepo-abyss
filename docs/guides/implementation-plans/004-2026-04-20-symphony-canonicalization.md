...
**Goal:** Canonicalize clinical-intelligence features currently owned by Dashboard or Assist into `packages/sentra/sentra-nada` per Chief-locked 7-phase order, closing the gaps surfaced in the 2026-04-20 coverage audit while keeping SYMPHONY as canonical parent and Dashboard/Assist as consumers.

**Architecture:** Pure TypeScript, zero runtime deps, vitest for tests, existing SYMPHONY engine pattern (deterministic functions exported from `packages/sentra/sentra-nada/src/engine/*.ts`, re-exported via `packages/sentra/sentra-nada/src/index.ts`). Every phase produces working-testable-software on its own and may be shipped independently.
...
- **No Assist source search** outside FEATURE.md citation. Assist-originating rules are ported by specification from FEATURE.md + existing SYMPHONY adapter (`packages/sentra/sentra-nada/src/adapters/assist-patterns-parity.ts`), not by reading Assist source code.
...
**Files:**
- Create: `packages/sentra/sentra-nada/src/engine/symptom-signals.ts`
- Create: `packages/sentra/sentra-nada/src/__tests__/symptom-signals.test.ts`
- Modify: `packages/sentra/sentra-nada/src/index.ts` (add exports)

### Task 1.1 — Type contract for symptom signals

**Files:**
- Create: `packages/sentra/sentra-nada/src/engine/symptom-signals.ts` (skeleton only)
- Test: `packages/sentra/sentra-nada/src/__tests__/symptom-signals.test.ts` (import + type test)

- [ ] **Step 1: Write failing test for public shape**

```typescript
// packages/sentra/sentra-nada/src/__tests__/symptom-signals.test.ts
...
Run: `pnpm --filter @sentra/nada test -- src/__tests__/symptom-signals.test.ts`
Expected: FAIL with "Module has no exported member 'detectSymphonySymptomSignals'".

- [ ] **Step 3: Create minimal skeleton**

```typescript
// packages/sentra/sentra-nada/src/engine/symptom-signals.ts
...
```typescript
// packages/sentra/sentra-nada/src/index.ts — append after vital-alerts export
export {
  detectSymphonySymptomSignals,
  type SymphonySymptomSignal,
  type SymphonySymptomSignalInput,
...
Run: `pnpm --filter @sentra/nada typecheck`
Expected: PASS.

- [ ] **Step 7: Commit**

```bash
git add packages/sentra/sentra-nada/src/engine/symptom-signals.ts \
        packages/sentra/sentra-nada/src/__tests__/symptom-signals.test.ts \
...
**Files:**
- Modify: `packages/sentra/sentra-nada/src/engine/symptom-signals.ts`
- Modify: `packages/sentra/sentra-nada/src/__tests__/symptom-signals.test.ts`
...
```typescript
// packages/sentra/sentra-nada/src/engine/symptom-signals.ts — replace body
...
4. Run test → PASS.
5. Run full file test suite: `pnpm --filter @sentra/nada test -- src/__tests__/symptom-signals.test.ts` — confirm no regression.
...
Run: `pnpm --filter @sentra/nada test`
...
Run: `pnpm --filter @sentra/nada lint`
Expected: PASS.
...
**Decision:** Ported as `packages/sentra/sentra-nada/src/engine/symptom-signals.ts`, pure TS, zero runtime deps, 19 signals, 3-token negation window.
...
- [ ] `packages/sentra/sentra-nada/src/engine/symptom-signals.ts` exists with 19 signal matchers + negation window.
- [ ] `packages/sentra/sentra-nada/src/__tests__/symptom-signals.test.ts` asserts at least 21 behaviors across positive, negated, and multi-token cases.
- [ ] `packages/sentra/sentra-nada/src/index.ts` re-exports `detectSymphonySymptomSignals`, `SymphonySymptomSignal`, `SymphonySymptomSignalInput`, `SymphonySymptomSignalResult`.
- [ ] `pnpm --filter @sentra/nada test` GREEN (count ≥ previous count + 21).
- [ ] `pnpm --filter @sentra/nada typecheck` PASS.
- [ ] `pnpm --filter @sentra/nada lint` PASS.
...
**Goal:** Port the generic rule evaluator engine from Assist `lib/emergency-detector/pattern-engine.ts` into `packages/sentra/sentra-nada/src/engine/pattern-matcher.ts`. Provides reusable AND/OR-counted evaluation with 8 operators and nested field resolution — the infrastructure that Phase 3 will stand on.

**Files (expected):**
- Create: `packages/sentra/sentra-nada/src/engine/pattern-matcher.ts`
- Create: `packages/sentra/sentra-nada/src/engine/pattern-matcher-types.ts` (or inline in matcher file)
- Create: `packages/sentra/sentra-nada/src/__tests__/pattern-matcher.test.ts`
...
**Before executing Phase 2:** Write detailed plan `docs/guides/implementation-plans/YYYY-MM-DD-symphony-phase-2-pattern-matcher.md` via fresh `/avcn-brainstorm` → `/avcn-write-plan` cycle.
...
- Adapter parity (existing `packages/sentra/sentra-nada/src/adapters/assist-patterns-parity.ts`) maps matched Assist output → canonical alert shape. It does NOT execute pattern matching.
...
**Goal:** Produce `packages/sentra/sentra-nada/src/engine/clinical-patterns.ts` that consumes Phase 1 (`detectSymphonySymptomSignals`) + Phase 2 (`matchSymphonyPattern`) and evaluates all 70 CP definitions natively.

**Files (expected):**
- Create: `packages/sentra/sentra-nada/src/engine/clinical-patterns.ts`
- Create: `packages/sentra/sentra-nada/src/engine/clinical-patterns-definitions.ts` (70 CP definitions as data, one-time port from FEATURE.md §2.1 spec — Chief-validated per gate group)
- Create: `packages/sentra/sentra-nada/src/__tests__/clinical-patterns.test.ts`
- Modify: `packages/sentra/sentra-nada/src/index.ts`
- Modify: `packages/sentra/sentra-nada/src/adapters/assist-patterns-parity.ts` (adapter must continue to work; evaluator runs alongside, does not replace adapter until Chief Phase E release gate GO)
...
**Before executing Phase 3:** Write detailed plan `docs/guides/implementation-plans/YYYY-MM-DD-symphony-phase-3-clinical-patterns.md`.
...
**Goal:** Port 9 ABCDE protocols (`PROTO_RESP_FAILURE`, `PROTO_SHOCK`, `PROTO_SEPSIS`, `PROTO_ANAPHYLAXIS`, `PROTO_ACS`, `PROTO_STROKE`, `PROTO_DKA_HHS`, `PROTO_HYPOGLYCEMIA`, `PROTO_CARDIAC_ARREST`) from Assist `action-protocols.ts` into `packages/sentra/sentra-nada/src/engine/action-protocols.ts` as canonical decision-support output templates.

**Files (expected):**
- Create: `packages/sentra/sentra-nada/src/engine/action-protocols.ts`
- Create: `packages/sentra/sentra-nada/src/__tests__/action-protocols.test.ts`
- Modify: `packages/sentra/sentra-nada/src/index.ts`
...
- Modify: `packages/shared/shared-types/src/symphony.ts` (add 3 union members)
- Modify: `packages/sentra/sentra-nada/src/engine/clinical-patterns.ts` (if Phase 3 already landed, CPs carrying these gates map correctly)
...
**Implemented scope:**
- Added `detectSymphonyTreatmentResponse()` to `packages/sentra/sentra-nada/src/engine/trajectory.ts`, using first-half vs second-half slope comparison on the worst worsening parameter.
...
- Canonicalized dashboard deterministic helpers into `packages/sentra/sentra-nada/src/engine/classifiers.ts`: chronic-disease classifier, hypertension classifier, glucose classifier, and AVPU/GCS mapper helpers.
...
**Files landed:**
- Modified: `packages/sentra/sentra-nada/src/engine/trajectory.ts`
- Created: `packages/sentra/sentra-nada/src/engine/classifiers.ts`
- Modified: `packages/sentra/sentra-nada/src/__tests__/trajectory.test.ts`
- Modified: `packages/sentra/sentra-nada/src/index.ts`
- Modified: `packages/shared/shared-types/src/symphony.ts`
...
- Verification 2026-04-22: `pnpm --filter @sentra/nada test` PASS 213/213, `typecheck` PASS, `lint` PASS, Dashboard `pnpm run test:symphony:route-parity` PASS 76/76 with gate still `partial`.
...
- Keep `traffic-light` in `@sentra/nada` as the canonical decision-safety gate.
...
Baseline (2026-04-20): `pnpm --filter @sentra/nada test` = 11 files / 57 tests GREEN.
...
- `packages/sentra/sentra-nada/src/contracts/index.ts`: no contract surface change in Phase 1 or 2. Phase 3 may introduce `assist-cp-XXX` alert IDs into a canonical alert-id registry. Phase 5 touches contract directly (gate taxonomy). Bump `SYMPHONY_CONTRACT_VERSION` accordingly.
...
- ❌ Touching `packages/platform/database`.
