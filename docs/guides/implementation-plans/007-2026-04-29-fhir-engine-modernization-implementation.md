# `packages/sentra/sentra-sandi` Modernization Implementation Plan
...
**Goal:** Modernize `packages/sentra/sentra-sandi` into an honest, bounded, R5-target validation and normalization candidate for AADI V2 Phase 2, without moving reasoning authority out of `@sentra/nada`.

**Architecture:** Keep all reasoning-driven mapping and interop intent in `packages/sentra/sentra-nada`. Modernize `packages/sentra/sentra-sandi` in place so it becomes truthful about what it supports today, explicit about what it does not support yet, and structurally ready for later promotion of validation/profile responsibilities.

**Tech Stack:** TypeScript strict, Vitest, Zod, pnpm workspace, `@sentra/nada` as canonical reasoning package, HL7 FHIR R5-target documentation posture, bounded R4/R5 transition language.
...
5. `docs/specs/aadi-v2/README.md`
6. `AGENTS.md`

## Non-Negotiable Constraints

- `SYMPHONY` remains the only clinical reasoning authority.
- Do not move `mapSymphonyResultToFhirBundle()` or `mapSymphonyResultToCdsHooksResponse()` into `packages/sentra/sentra-sandi` in this plan.
...
This plan assumes `packages/sentra/sentra-sandi` currently contains:
...
**Files:**
- Create/modify: `packages/sentra/sentra-sandi/src/__tests__/modernization-baseline.test.ts`
...
```bash
git add packages/sentra/sentra-sandi/src/__tests__/modernization-baseline.test.ts
...
- explicitly say AADI V2 interop mapping authority remains in `packages/sentra/sentra-nada`
...
```bash
git add packages/sentra/sentra-sandi/README.md packages/sentra/sentra-sandi/src/index.ts
...
**Files:**
- Modify: `packages/sentra/sentra-sandi/src/transformer.ts`
- Modify/create: `packages/sentra/sentra-sandi/src/__tests__/transformer.test.ts`
- Modify: `packages/sentra/sentra-sandi/src/index.ts` if export comments need to change further
...
```bash
git add packages/sentra/sentra-sandi/src/transformer.ts \
        packages/sentra/sentra-sandi/src/__tests__/transformer.test.ts \
...
```bash
git add packages/sentra/sentra-sandi/README.md \
        packages/sentra/sentra-sandi/src/types.ts \
...
- Modify: `packages/sentra/sentra-sandi/README.md`
- Modify: `packages/sentra/sentra-sandi/src/index.ts`
- Optionally add: `packages/sentra/sentra-sandi/src/version-strategy.ts`
...
```bash
git add packages/sentra/sentra-sandi/README.md packages/sentra/sentra-sandi/src/index.ts packages/sentra/sentra-sandi/src/version-strategy.ts
...
**Purpose:** Prepare a clean seam for future promotion from `packages/sentra/sentra-nada/src/interop/` without moving mapping authority now.

**Files:**
- Create: `packages/sentra/sentra-sandi/src/validation-hooks.ts`
- Modify: `packages/sentra/sentra-sandi/src/index.ts`
- Create: `packages/sentra/sentra-sandi/src/__tests__/validation-hooks.test.ts`
...
- Optional: add a short package-level modernization note in `packages/sentra/sentra-sandi/README.md`
...
- whether `packages/sentra/sentra-sandi` is now ready for the next promotion step
...
- interop mapping stays in `packages/sentra/sentra-nada`: preserved throughout
