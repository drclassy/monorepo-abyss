...
**Goal:** Formalize the AADI V2 CDS Hooks surface in `@sentra/nada` so its workflow contract is explicit, test-pinned, PHI-safe, and ready for future boundary decisions without prematurely promoting it into `packages/sentra/sentra-sandi`.

**Architecture:** Keep `packages/sentra/sentra-nada` as the owner of CDS workflow semantics, card ordering, hook-trigger posture, and `SymphonyResult` interpretation. Add a thin internal contract layer around discovery metadata, hook context, prefetch assumptions, and response invariants so the current card adapter stops being a loose stub and becomes a documented, test-backed workflow surface.

**Tech Stack:** TypeScript strict, Vitest, pnpm workspace, HL7 CDS Hooks v2.0.1 contract posture, existing `@sentra/nada` interop layer.
...
12. `packages/sentra/sentra-nada/src/interop/symphony-to-cds-hooks.ts`
13. `packages/sentra/sentra-nada/src/__tests__/symphony-to-cds-hooks.test.ts`
...
- Do not move `mapSymphonyResultToCdsHooksResponse()` into `packages/sentra/sentra-sandi` in this plan.
...
- [ ] `corepack pnpm --filter @sentra/nada test`
- [ ] `corepack pnpm --filter @sentra/nada typecheck`

If documentation or package-boundary decisions are updated:

- [ ] `corepack pnpm --filter @sentra/sandi test`
- [ ] `corepack pnpm --filter @sentra/sandi typecheck`
...
- `mapSymphonyResultToCdsHooksResponse()` is a deterministic card emitter in `packages/sentra/sentra-nada/src/interop/symphony-to-cds-hooks.ts`
...
**Files:**
- Modify: `packages/sentra/sentra-nada/src/__tests__/symphony-to-cds-hooks.test.ts`
...
- no accidental dependency on `packages/sentra/sentra-sandi` is introduced

- [ ] **Step 3: Run the focused test file**

```bash
corepack pnpm --filter @sentra/nada test -- src/__tests__/symphony-to-cds-hooks.test.ts
```
...
```bash
git add packages/sentra/sentra-nada/src/__tests__/symphony-to-cds-hooks.test.ts
...
**Files:**
- Create: `packages/sentra/sentra-nada/src/interop/cds-hooks-contract.ts`
- Modify: `packages/sentra/sentra-nada/src/interop/index.ts`
- Modify: `packages/sentra/sentra-nada/src/index.ts`
- Test: `packages/sentra/sentra-nada/src/__tests__/cds-hooks-contract.test.ts`
...
```bash
corepack pnpm --filter @sentra/nada test -- src/__tests__/cds-hooks-contract.test.ts
```
...
```bash
git add packages/sentra/sentra-nada/src/interop/cds-hooks-contract.ts \
        packages/sentra/sentra-nada/src/interop/index.ts \
        packages/sentra/sentra-nada/src/index.ts \
        packages/sentra/sentra-nada/src/__tests__/cds-hooks-contract.test.ts
...
**Files:**
- Create: `packages/sentra/sentra-nada/src/interop/cds-hooks-service-definition.ts`
- Modify: `packages/sentra/sentra-nada/src/interop/index.ts`
- Modify: `packages/sentra/sentra-nada/src/index.ts`
- Test: `packages/sentra/sentra-nada/src/__tests__/cds-hooks-service-definition.test.ts`
...
```bash
corepack pnpm --filter @sentra/nada test -- src/__tests__/cds-hooks-service-definition.test.ts
```
...
```bash
git add packages/sentra/sentra-nada/src/interop/cds-hooks-service-definition.ts \
        packages/sentra/sentra-nada/src/interop/index.ts \
        packages/sentra/sentra-nada/src/index.ts \
        packages/sentra/sentra-nada/src/__tests__/cds-hooks-service-definition.test.ts
...
**Files:**
- Modify: `packages/sentra/sentra-nada/src/interop/symphony-to-cds-hooks.ts`
- Create: `packages/sentra/sentra-nada/src/interop/cds-hooks-card-policy.ts`
- Modify: `packages/sentra/sentra-nada/src/interop/index.ts`
- Test: `packages/sentra/sentra-nada/src/__tests__/symphony-to-cds-hooks.test.ts`
- Test: `packages/sentra/sentra-nada/src/__tests__/cds-hooks-card-policy.test.ts`
...
```bash
corepack pnpm --filter @sentra/nada test -- src/__tests__/symphony-to-cds-hooks.test.ts
corepack pnpm --filter @sentra/nada test -- src/__tests__/cds-hooks-card-policy.test.ts
```
...
```bash
git add packages/sentra/sentra-nada/src/interop/symphony-to-cds-hooks.ts \
        packages/sentra/sentra-nada/src/interop/cds-hooks-card-policy.ts \
        packages/sentra/sentra-nada/src/interop/index.ts \
        packages/sentra/sentra-nada/src/__tests__/symphony-to-cds-hooks.test.ts \
        packages/sentra/sentra-nada/src/__tests__/cds-hooks-card-policy.test.ts
...
**Files:**
- Create: `docs/specs/aadi-v2/014-2026-04-29-aadi-v2-cds-hooks-formalization.md`
- Modify: `docs/specs/aadi-v2/README.md`
...
Add the new formalization spec and plan to `docs/specs/aadi-v2/README.md` in reading order and document map.
...
```bash
git add docs/specs/aadi-v2/014-2026-04-29-aadi-v2-cds-hooks-formalization.md \
...
**Files:**
- Modify: `docs/specs/aadi-v2/014-2026-04-29-aadi-v2-cds-hooks-formalization.md`
- Modify: `.agent/sessions/2026-04-29.md`
...
- `mapSymphonyResultToCdsHooksResponse()` remains in `packages/sentra/sentra-nada`
