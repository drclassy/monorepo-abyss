...
**Primary package:** `packages/sentra/sentra-nada/`  
**Related inputs:** `docs/guides/004-aadi-v2.md`, `.agent/HANDOFF.md`, `docs/adr/0007-pharmacology-locus-decision.md`, `docs/specs/aadi-v2/005-2026-04-27-aadi-v2-feature-coverage-matrix.md`
...
`@sentra/nada`, which already contains a meaningful deterministic
...
- `packages/sentra/sentra-nada/src/engine/assess.ts` still accepts
...
This matches the central claim in `docs/guides/004-aadi-v2.md`: current SYMPHONY is
...
1. Turn `@sentra/nada` into a native diagnostic reasoning engine without
...
- all new reasoning modules live in `packages/sentra/sentra-nada`
...
- parity and regression tests already present in `packages/sentra/sentra-nada/src/__tests__/`
...
`packages/sentra/sentra-nada`, that duplication is a design failure unless the spec is
...
|---|---|---|---|---|
| Assessment orchestrator | `packages/sentra/sentra-nada/src/engine/assess.ts` | Remains canonical entrypoint; gains native reasoning path instead of being replaced | Sprint 2 | MUST reuse |
| Shared contracts | `packages/shared/shared-types/src/symphony.ts` | Remains the public package boundary; new AADI V2 contracts append here | Sprint 2 | MUST reuse |
| NEWS2 | `packages/sentra/sentra-nada/src/engine/news2.ts` | Continues as canonical deterioration score and safety input | Sprint 1-2 | MUST reuse |
| Vital alerts | `packages/sentra/sentra-nada/src/engine/vital-alerts.ts` | Continues as hard-stop bedside alert source | Sprint 1-2 | MUST reuse |
| Screening gates | `packages/sentra/sentra-nada/src/engine/screening-gates.ts` | Continues as deterministic immediate screening layer | Sprint 1-2 | MUST reuse |
| Symptom NLP | `packages/sentra/sentra-nada/src/engine/symptom-signals.ts` | Becomes one of the first feeders into `ClinicalFacts` and syndrome classification | Sprint 1 | MUST reuse |
| Pattern evaluator | `packages/sentra/sentra-nada/src/engine/pattern-engine.ts` | Continues as deterministic criteria engine for diagnosis packs and alert packs where applicable | Sprint 1 | MUST reuse |
| 70 clinical patterns | `packages/sentra/sentra-nada/src/engine/clinical-patterns.ts`, `packages/sentra/sentra-nada/src/engine/clinical-patterns-definitions.ts` | Continues as canonical disease-pattern and must-not-miss evidence source | Sprint 1-2 | MUST reuse |
| Assist pattern parity definitions | `packages/sentra/sentra-nada/src/adapters/assist-patterns-parity.ts` | Remains migration guard and no-regression evidence for Assist-origin features | Sprint 3 | MUST reuse |
| PE suspect | `packages/sentra/sentra-nada/src/engine/pe-suspect.ts` | Continues as focused embolic risk detector feeding safety arbiter and differential context | Sprint 1-2 | MUST reuse |
| Anaphylaxis detector | `packages/sentra/sentra-nada/src/engine/anaphylaxis.ts` | Continues as high-priority syndrome detector and action trigger | Sprint 1-2 | MUST reuse |
| Early warning patterns | `packages/sentra/sentra-nada/src/engine/early-warning.ts` | Continues as disease-specific warning source into facts, safety, and explanation | Sprint 1-2 | MUST reuse |
| Composite deterioration | `packages/sentra/sentra-nada/src/engine/composite-deterioration.ts` | Continues as multi-signal deterioration evidence and acute syndrome source | Sprint 1-2 | MUST reuse |
| Trajectory, baseline, momentum, response | `packages/sentra/sentra-nada/src/engine/trajectory.ts` | Continues as temporal reasoning layer and explanation input | Sprint 1-2 | MUST reuse |
| HTN/glucose/chronic disease/GCS classifiers | `packages/sentra/sentra-nada/src/engine/classifiers.ts` | Continue as canonical physiologic and chronic-context derivation utilities for facts builder | Sprint 1 | MUST reuse |
| Traffic-light safety gate | `packages/sentra/sentra-nada/src/engine/traffic-light.ts` | Remains final escalation gate inside SYMPHONY | Sprint 2 | MUST reuse |
| Action protocols | `packages/sentra/sentra-nada/src/engine/action-protocols.ts` | Remain canonical action guidance attached to alerts and surfaced in AADI V2 outputs | Sprint 2 | MUST reuse |
| Hybrid decisioning | `packages/sentra/sentra-nada/src/engine/hybrid-decisioning.ts` | Remains shadow/fallback/comparison path during migration | Sprint 2-3 | MUST reuse temporarily |
| Parity fixtures | `packages/sentra/sentra-nada/src/engine/parity-fixtures.ts` | Remain regression guard against historical route parity loss | Sprint 3 | MUST reuse |
...
`packages/shared/shared-types/src/symphony.ts`. Internal engines and registries stay in
...
### New / Expanded Files in `packages/sentra/sentra-nada`
...
`packages/clinical/clinical-references` remains:
...
- existing `packages/sentra/sentra-nada/src/__tests__/*`
