...
**Owner:** Codex/Dexton  
**Primary scope:** `packages/symphony/src/interop/`, `packages/fhir-engine/`  
**Related docs:**  
...
- `docs/superpowers/specs/2026-04-27-aadi-v2-design.md`  
...
> **Apakah interoperability AADI V2 tetap di `packages/symphony/src/interop/`, atau mulai dipromosikan ke `packages/fhir-engine`?**
...
- `packages/fhir-engine` tidak diangkat prematur padahal masih R4/TODO-heavy
...
### What `packages/symphony` Already Has

Saat ini `packages/symphony/src/interop/` sudah menyediakan:
...
### What `packages/fhir-engine` Currently Is
...
1. `packages/symphony/src/interop/` tetap menjadi rumah adapter shape-only saat ini
2. `packages/fhir-engine` dipersiapkan sebagai rumah validation/profile/normalization layer berikutnya
3. mapping authority tetap berasal dari `SymphonyResult`, bukan dari `fhir-engine`

### Rejected Alternatives

#### Alternative A — Move everything into `packages/fhir-engine` now
...
#### Alternative B — Keep everything forever inside `packages/symphony`
...
- current state
- adapters tetap di `packages/symphony/src/interop/`
...
- still inside `packages/symphony` or `shared-types`
...
### Stage P2 — Prepare `packages/fhir-engine` for R5-Aware Role

Goal:

- make `packages/fhir-engine` promotion-worthy
...
1. biarkan `packages/symphony/src/interop/` tetap hidup sebagai adapter deterministic
2. siapkan `packages/fhir-engine` supaya layak menerima responsibility validation
...
Write a focused modernization spec for `packages/fhir-engine`:
...
If promotion prep touches `packages/fhir-engine` code:
...
If interop adapters in `packages/symphony` are changed:
...
> **`packages/fhir-engine` is a candidate target, not the current canonical home of AADI V2 interop.**
...
1. `packages/fhir-engine modernization spec`
