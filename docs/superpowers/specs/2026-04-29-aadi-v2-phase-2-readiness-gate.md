...
**Owner:** Codex/Dexton  
**Primary scope:** `packages/symphony/`, `packages/fhir-engine/`, consumer bridge boundary  
**Related docs:**  
...
1. adapter `FHIR-ish` di `packages/symphony/src/interop/` dipromosikan
2. `packages/fhir-engine` ikut masuk sebagai dependency arsitektur Phase 2
...
3. HL7 terminology module tetap menegaskan terminology sebagai module
   ...
   Relevance: `packages/fhir-engine` promotion butuh terminology plan,
...
- native AADI V2 reasoning di `packages/symphony`
- shadow comparison
- parity verification
- interop stubs di `packages/symphony/src/interop/`
...
`packages/fhir-engine` saat ini masih berorientasi **FHIR R4**:
...
- jangan salah mengira bahwa keberadaan `packages/fhir-engine` berarti Phase 2 sudah siap
...
1. `packages/symphony` test suite hijau penuh pada baseline target branch.
...
2. `packages/symphony/src/interop/` tetap read-only terhadap `SymphonyResult`.
...
1. Harus ada keputusan apakah validation dilakukan:
   - inline di `packages/symphony`
   - di package sibling
   - atau di consumer boundary
2. Rekomendasi: validation **tidak** ditanam di `SYMPHONY` core.
3. Kalau `packages/fhir-engine` akan dipromosikan, harus ada keputusan eksplisit:
   - upgrade R4 -> R5
...
1. `packages/fhir-engine` masih dianggap canonical hanya karena namanya cocok,
...
- apakah `packages/fhir-engine` dipakai atau tidak
- kalau dipakai, bagaimana upgrade path-nya
- kalau tidak, apakah dibuat package baru atau tetap di `packages/symphony/src/interop/`
...
- upgrade atau refactor `packages/fhir-engine` bila memang dipilih
...
- tetap treat `packages/fhir-engine` sebagai candidate, bukan canonical target
...
Dan bila `packages/fhir-engine` disentuh:
...
- package promotion decision untuk `packages/fhir-engine` belum final
