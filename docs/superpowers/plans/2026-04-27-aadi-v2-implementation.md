...
**Goal:** Evolve `@the-abyss/symphony` into the AADI V2 native diagnostic reasoning engine while reusing every relevant existing SYMPHONY clinical feature, preserving current safety authority, and keeping Dashboard/Assist as consumers.

**Architecture:** Build AADI V2 additively inside `packages/symphony` using the existing deterministic stack as foundation. Sprint 1 creates the native reasoning spine from reused snapshot/symptom/pattern/composite/trajectory/classifier outputs. Sprint 2 turns that spine into clinician-usable output through arbiter, explainability, and clinical disposition while preserving alerts, protocols, trajectory, and traffic-light. Sprint 3 adds shadow comparison and regression gates. Sprint 4 adds consumer-facing interoperability adapters without shifting clinical authority out of SYMPHONY.

**Tech Stack:** TypeScript strict, vitest, pnpm workspace, `@the-abyss/shared-types`, existing `packages/symphony` engine modules, `packages/clinical-references` as reference-only sibling package.

---

## Baseline Documents (MUST READ before implementing)

1. `docs/superpowers/specs/2026-04-27-aadi-v2-design.md`
2. `docs/superpowers/specs/2026-04-27-aadi-v2-feature-coverage-matrix.md`
3. `.agent/FEATURE.md`
4. `.agent/HANDOFF.md`
5. `AGENTS.md`
...
- [ ] Open `docs/superpowers/specs/2026-04-27-aadi-v2-feature-coverage-matrix.md`
...
This plan assumes the following additions inside `packages/symphony`:
...
**Files:**
- Modify: `packages/shared-types/src/symphony.ts`
- Modify: `packages/shared-types/src/index.ts`
- Modify: `packages/symphony/src/index.ts`
- Test: `packages/symphony/src/__tests__/contract.test.ts`

- [ ] **Step 1: Add failing contract tests for new AADI V2 output shape**

```typescript
// Append to packages/symphony/src/__tests__/contract.test.ts
...
Run: `pnpm --filter @the-abyss/symphony test -- src/__tests__/contract.test.ts`
...
```typescript
// Replace the SymphonyResult interface block in packages/shared-types/src/symphony.ts
...
```typescript
// packages/shared-types/src/index.ts already exports './symphony' — no new line needed if unchanged.
// packages/symphony/src/index.ts — append in the contract export block
...
Run:
- `pnpm --filter @the-abyss/symphony test -- src/__tests__/contract.test.ts`
- `pnpm --filter @the-abyss/symphony typecheck`
...
```bash
git add packages/shared-types/src/symphony.ts \
        packages/shared-types/src/index.ts \
...
**Files:**
- Create: `packages/symphony/src/engine/clinical-facts.ts`
- Create: `packages/symphony/src/__tests__/clinical-facts.test.ts`
- Modify: `packages/symphony/src/index.ts`
...
Run: `pnpm --filter @the-abyss/symphony test -- src/__tests__/clinical-facts.test.ts`
...
Run:
- `pnpm --filter @the-abyss/symphony test -- src/__tests__/clinical-facts.test.ts`
- `pnpm --filter @the-abyss/symphony typecheck`
...
```bash
git add packages/symphony/src/engine/clinical-facts.ts \
        packages/symphony/src/__tests__/clinical-facts.test.ts \
        packages/symphony/src/index.ts
...
**Files:**
- Create: `packages/symphony/src/engine/syndrome-classifier.ts`
- Create: `packages/symphony/src/__tests__/syndrome-classifier.test.ts`
- Modify: `packages/symphony/src/index.ts`
...
Run: `pnpm --filter @the-abyss/symphony test -- src/__tests__/syndrome-classifier.test.ts`
...
Run:
- `pnpm --filter @the-abyss/symphony test -- src/__tests__/syndrome-classifier.test.ts`
- `pnpm --filter @the-abyss/symphony typecheck`

Commit:

```bash
git add packages/symphony/src/engine/syndrome-classifier.ts \
        packages/symphony/src/__tests__/syndrome-classifier.test.ts \
        packages/symphony/src/index.ts
...
**Files:**
- Create: `packages/symphony/src/engine/diagnosis-packs.ts`
- Create: `packages/symphony/src/engine/native-differential.ts`
- Create: `packages/symphony/src/__tests__/native-differential.test.ts`
- Modify: `packages/symphony/src/index.ts`
...
Run: `pnpm --filter @the-abyss/symphony test -- src/__tests__/native-differential.test.ts`
Expected: FAIL.
...
Run:
- `pnpm --filter @the-abyss/symphony test -- src/__tests__/native-differential.test.ts`
- `pnpm --filter @the-abyss/symphony typecheck`

Commit:

```bash
git add packages/symphony/src/engine/diagnosis-packs.ts \
        packages/symphony/src/engine/native-differential.ts \
        packages/symphony/src/__tests__/native-differential.test.ts \
        packages/symphony/src/index.ts
...
**Files:**
- Create: `packages/symphony/src/engine/reasoning-arbiter.ts`
- Create: `packages/symphony/src/__tests__/reasoning-arbiter.test.ts`
- Modify: `packages/symphony/src/index.ts`
...
Run:
- `pnpm --filter @the-abyss/symphony test -- src/__tests__/reasoning-arbiter.test.ts`
- `pnpm --filter @the-abyss/symphony typecheck`

Commit:

```bash
git add packages/symphony/src/engine/reasoning-arbiter.ts \
        packages/symphony/src/__tests__/reasoning-arbiter.test.ts \
        packages/symphony/src/index.ts
...
**Files:**
- Create: `packages/symphony/src/engine/explainability.ts`
- Create: `packages/symphony/src/engine/confidence-engine.ts`
- Create: `packages/symphony/src/__tests__/confidence-engine.test.ts`
- Modify: `packages/symphony/src/index.ts`
...
Run:
- `pnpm --filter @the-abyss/symphony test -- src/__tests__/confidence-engine.test.ts`
- `pnpm --filter @the-abyss/symphony typecheck`

Commit:

```bash
git add packages/symphony/src/engine/explainability.ts \
        packages/symphony/src/engine/confidence-engine.ts \
        packages/symphony/src/__tests__/confidence-engine.test.ts \
        packages/symphony/src/index.ts
...
**Files:**
- Modify: `packages/symphony/src/engine/assess.ts`
- Create: `packages/symphony/src/__tests__/aadi-v2.integration.test.ts`
...
```typescript
// packages/symphony/src/engine/assess.ts — add imports
...
```typescript
// packages/symphony/src/engine/assess.ts — inside assessSymphonyInput before return
...
```typescript
// packages/symphony/src/engine/assess.ts — replace return metadata/result sections
    metadata: {
...
Run:
- `pnpm --filter @the-abyss/symphony test -- src/__tests__/aadi-v2.integration.test.ts`
- `pnpm --filter @the-abyss/symphony test`
- `pnpm --filter @the-abyss/symphony typecheck`
- `pnpm --filter @the-abyss/symphony lint`
...
```bash
git add packages/symphony/src/engine/assess.ts \
        packages/symphony/src/__tests__/aadi-v2.integration.test.ts
...
**Files:**
- Create: `packages/symphony/src/engine/shadow-compare.ts`
- Create: `packages/symphony/src/__tests__/shadow-compare.test.ts`
- Modify: `packages/symphony/src/engine/assess.ts`
- Modify: `packages/symphony/src/index.ts`
...
```typescript
// packages/symphony/src/engine/assess.ts — add import
import { compareSymphonyShadowPaths } from './shadow-compare'
```

```typescript
// packages/symphony/src/engine/assess.ts — before return
...
```typescript
// packages/symphony/src/engine/assess.ts — add to return
    shadowComparison,
```
...
Run:
- `pnpm --filter @the-abyss/symphony test -- src/__tests__/shadow-compare.test.ts`
- `pnpm --filter @the-abyss/symphony test`
- `pnpm --filter @the-abyss/symphony typecheck`
...
```bash
git add packages/symphony/src/engine/shadow-compare.ts \
        packages/symphony/src/__tests__/shadow-compare.test.ts \
...
**Files:**
- Modify: `packages/symphony/src/__tests__/aadi-v2.integration.test.ts`
- Modify: `packages/symphony/src/engine/parity-fixtures.ts` (only if helper export is needed)

- [ ] **Step 1: Add failing verification smoke test for parity suites**

```typescript
// Append to packages/symphony/src/__tests__/aadi-v2.integration.test.ts
import {
...
```bash
git add packages/symphony/src/__tests__/aadi-v2.integration.test.ts \
...
**Files:**
- Create: `packages/symphony/src/engine/interoperability.ts`
- Modify: `packages/symphony/src/index.ts`
...
export function mapSymphonyResultToInteroperabilityEnvelope(
  ...
  return {
    resourceType: 'GuidanceResponse',
    status: 'success',
    moduleUri: '@the-abyss/symphony/aadi-v2',
    subject: {
...
```bash
git add packages/symphony/src/engine/interoperability.ts \
        packages/symphony/src/index.ts \
        packages/symphony/src/__tests__/contract.test.ts
...
Every major feature group from `docs/superpowers/specs/2026-04-27-aadi-v2-feature-coverage-matrix.md` is represented by at least one task:
