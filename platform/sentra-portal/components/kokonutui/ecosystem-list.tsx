...
export default function EcosystemList({ totalNodes = "12 Intelligence Nodes", divisions = DIVISIONS, className }: EcosystemListProps) {
  return (
    <div
      className={cn(
    ...
    >
      ...
      <div className="p-3">
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-xs font-medium text-zinc-900 dark:text-zinc-100">Regional Sub-Systems</h2>
        </div>
          ...
          {divisions.map((div) => (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                >
                  {div.type === "healthcare" && <Heart className="w-3.5 h-3.5 text-rose-600 dark:text-rose-400" />}
                  {div.type === "platform" && <Cpu className="w-3.5 h-3.5 text-blue-600 dark:text-blue-400" />}
                  {div.type === "library" && <BookOpen className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" />}
                  {div.type === "agent" && <Brain className="w-3.5 h-3.5 text-purple-600 dark:text-purple-400" />}
                </div>
                <div>
                  <h3 className="text-xs font-medium text-zinc-900 dark:text-zinc-100">{div.title}</h3>
                  {div.description && (
                    <p className="text-[11px] text-zinc-600 dark:text-zinc-400">{div.description}</p>
                  )}
                </div>
              </div>

              <div className="text-right">
                <span className="text-xs font-medium text-zinc-900 dark:text-zinc-100">{div.status}</span>
              </div>
            </div>
          ))}
        </div>
