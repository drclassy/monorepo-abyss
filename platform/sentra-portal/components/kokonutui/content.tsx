...
import { Activity, Server, Zap, Database, BookOpen } from 'lucide-react'
...
export default function Content() {
  ...
  const SECTION_TITLE =
    'text-[13px] font-medium text-zinc-500 mb-6 text-left flex items-center gap-2 uppercase tracking-[0.2em]'
        ...
        <div className="bg-white dark:bg-[#0F0F12] rounded-xl p-6 flex flex-col border border-gray-200 dark:border-[#1F1F23]">
          ...
          <div className="space-y-3">
            {[
              ...
              {
                id: 'S3',
                name: 'Aby Engine (Gemma 2)',
                status: 'GOVERNING',
                activity: 'Gemma 2 9B',
              },
              {
                id: 'S4',
                name: 'Orchestrator Saga Node',
                status: 'STABLE',
                activity: '98.2% Sync',
              },
            ].map((item) => (
              <div
                key={item.id}
                className="flex items-center justify-between p-3 rounded-lg bg-zinc-50 dark:bg-zinc-900/30 border border-zinc-100 dark:border-zinc-800 transition-all hover:border-zinc-300 dark:hover:border-zinc-700"
              >
                <div className="text-left">
                  <h4 className={ITEM_TITLE}>{item.name}</h4>
                  <p className={ITEM_PANTONE} style={{ color: PANTONE_RED }}>
                    {item.activity}
                  </p>
                </div>
                <span
        ...
        <div className="bg-white dark:bg-[#0F0F12] rounded-xl p-6 flex flex-col border border-gray-200 dark:border-[#1F1F23]">
          ...
          <div className="grid grid-cols-1 gap-4">
            {[
              ...
              <div
                key={i}
                className="flex items-center justify-between p-2.5 border-b border-zinc-100 dark:border-zinc-800 last:border-0"
              >
                <div className="text-left">
                ...
                <div className={cn(ITEM_SUBTITLE, 'font-bold uppercase tracking-widest')}>
                  {api.throughput}
                </div>
              </div>
            ))}
      ...
      <div className="bg-white dark:bg-[#0F0F12] rounded-xl p-6 border border-gray-200 dark:border-[#1F1F23]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            ...
            <div
              key={i}
              className="flex flex-col text-left p-4 rounded-lg bg-zinc-50 dark:bg-zinc-900/30 border border-zinc-100 dark:border-zinc-800"
            >
              <stat.icon className="w-3.5 h-3.5 text-zinc-400 mb-2" />
              <span className={cn(ITEM_SUBTITLE, 'font-bold uppercase tracking-tighter')}>
                {stat.label}
              </span>
              <span className="text-[21px] font-bold text-zinc-900 dark:text-zinc-50 mt-1">
                {stat.value}
              </span>
              <p className={ITEM_PANTONE + ' mt-0.5'} style={{ color: PANTONE_RED }}>
                {stat.sub}
              </p>
            </div>
          ))}
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
              {[
                {
                  node: 'Aby (Gemma 2)',
                  task: 'Clinical Knowledge Governance',
                  duration: '1.2s',
                  status: 'GOVERNING',
                },
                {
                  node: 'Jen Governor',
                  task: 'Root Monorepo Cleanup & Integrity Scan',
                  duration: '15m',
                  status: 'DONE',
                },
                {
                ...
                <tr key={i} className="group transition-all">
                  ...
                  <td
                    className={cn(ITEM_PANTONE, 'py-4 text-center font-bold')}
                    style={{ color: PANTONE_RED }}
                  >
                    {log.duration}
                  </td>
                  <td className="py-4 text-right">
                    <button
                      className={cn(
                        BUTTON_TEXT,
                        'px-3 py-1 rounded bg-white dark:bg-zinc-200 text-zinc-900 font-bold shadow-[2px_2px_5px_rgba(0,0,0,0.1)] opacity-0 group-hover:opacity-100 transition-all font-sans'
                      )}
                    >
                      INTERVENE
                    </button>
