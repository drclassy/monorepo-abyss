...
| Path | Notes |
|------|-------|
| `apps/community/classy-memory/.claude/settings.local.json` | ⚠️ Contains live key-like local config markers `[REDACTED_KEY_EXAMPLE]` + `[REDACTED_TOKEN_EXAMPLE]` — already ignored by child `.gitignore:54` (`/.claude/`). **Action: rotate keys, not a git issue.** |
...
**Real secret value scan** (patterns: `[REDACTED_SECRET_PATTERN_EXAMPLE]`, `[REDACTED_TOKEN_EXAMPLE]`, `[REDACTED_KEY_EXAMPLE]`):
- 11 matches in `apps/prototype/agent-hermes/vendor/` — all TEST FIXTURES / DOCS EXAMPLES (hindsight, hermes-core vendored modules)
- 1 match in `apps/community/classy-transformer/website/public/slack-logo-icon.jpg` — JPEG binary bytes coincidentally match regex (false positive)
...
20+ files match `([REDACTED_SECRET_PATTERN_EXAMPLE])` in path — ALL are feature code (auth flows, telemedicine tokens, api-key guards, reset-password routes). **Zero secrets; this is expected feature nomenclature.**
