...
### [2026-05-16] `.agent/` continuity protocol is mandatory for all agents
**Context:** During monorepo stabilization, Codex correctly read `AGENTS.md` but did not treat `.agent/` as the required operational SSOT before continuing technical work. Chief corrected the model: `AGENTS.md` is the public rulebook, while `.agent/` holds continuity, working digest, guardrails, handoffs, operational memory, and workflow state.
**Decision:**
1. `.agent/` is protected operational SSOT for Sentra / ABYSS work.
2. Every agent must verify `.agent/` exists and load active `.agent/` state before substantive repo work.
3. The minimum live startup set is `DIGEST.md` when present, then `SESSION_STATE.md`, `HANDOFF.md`, `CONTEXT.md`, `PROGRESS.md`, `LESSONS.md`, and `DECISIONS.md` as needed for the task.
4. `.agent/` must not be deleted, moved, cleaned, reset, treated as cache/junk, or replaced by `AGENTS.md`.
5. Agents may read `.agent/` by default. Agents may update specific `.agent/` files only when explicitly requested or when a session-end/progress rule requires it.
**Rejected alternatives:** Treating `AGENTS.md` as sufficient continuity context; relying on ChatGPT memory as repo SSOT; placing operational state only in external agent memory; deleting or ignoring `.agent/` as generated cache.
**Rationale:** Sentra / ABYSS continuity depends on auditable local state that survives across tools and agents. Keeping operational state inside `.agent/` prevents lost decisions, stale handoffs, and repeated context drift.
**Consequences:** Startup hooks, digest generation, handoff updates, and agent final reports must preserve the distinction: `AGENTS.md` = rulebook, `.agent/` = live operational state.
