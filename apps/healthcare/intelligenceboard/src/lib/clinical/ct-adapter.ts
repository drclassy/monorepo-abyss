import type {
  ...
  ClinicalTrajectorySymptomPoint,
...
import type { MomentumAnalysis, TrajectoryAnalysis, VisitRecord } from './trajectory-analyzer'
...
function mapVisitSource(source: VisitRecord['source']): 'manual' | 'imported' {
  return source === 'scrape' ? 'imported' : 'manual'
}
...
function buildVitalsTimeline(visits: VisitRecord[]): ClinicalTrajectoryVitalPoint[] {
  return visits.map((v, i) => ({
    ...
    source: mapVisitSource(v.source),
    sbp: v.vitals.sbp,
    dbp: v.vitals.dbp,
    hr: v.vitals.hr,
    rr: v.vitals.rr,
...
function buildSymptomsTimeline(
  visits: VisitRecord[]
): ClinicalTrajectorySymptomPoint[] | undefined {
  const symptomsTimeline = visits.flatMap((v, i) => {
    const symptom = v.keluhan_utama.trim()
    if (!symptom) return []

    return [
      {
        id: v.encounter_id ? `sp-${v.encounter_id}` : `sp-${i}`,
        observedAt: v.timestamp,
        source: mapVisitSource(v.source),
        symptom,
      },
  ...
  return symptomsTimeline.length > 0 ? symptomsTimeline : undefined
}
...
function buildDerivedTimeline(
  ...
  if (gcsEvents && gcsEvents.length > 0) {
    ...
    const slopeFlag =
      slopePerHour !== undefined ? `t49:slope:${slopePerHour.toFixed(2)}` : 't49:slope:unknown'
    derived.push({
...
function buildResponse(
  ...
  return {
    ...
    nextBestClinicalCheck: getNextBestClinicalCheck(analysis),
    nextBestClinicalQuestion: getNextBestClinicalQuestion(analysis),
    recommendedMonitoringCadence: analysis.clinical_safe_output.review_window,
  }
}

function isQuestionLikeRecommendation(text: string): boolean {
  return (
    text.endsWith('?') ||
    /^(apakah|bagaimana|kapan|kenapa|mengapa|berapa|adakah|siapa|what|when|why|how|is|are|do|does|can|should)\b/i.test(
      text
    )
  )
}

function getNormalizedRecommendations(analysis: TrajectoryAnalysis): string[] {
  return analysis.recommendations
    .map((recommendation) => recommendation.text.trim())
    .filter((text) => text.length > 0)
}

function getNextBestClinicalCheck(analysis: TrajectoryAnalysis): string | undefined {
  return getNormalizedRecommendations(analysis)[0]
}

function getNextBestClinicalQuestion(analysis: TrajectoryAnalysis): string | undefined {
  return getNormalizedRecommendations(analysis).find(isQuestionLikeRecommendation)
}
...
export function legacyIBToCtV1(
  ...
  const treatmentTimeline =
    treatments && treatments.length > 0 ? buildTreatmentTimeline(treatments, visits) : undefined
  const labsTimeline = labs && labs.length > 0 ? buildLabsTimeline(labs) : undefined
  ...
  const symptomsTimeline = buildSymptomsTimeline(visits)
  return {
    version: 'ct.v1',
