...
test('NEWS2 score for critical respiratory visit is ≥9', () => {
  ...
  assert.ok(
    lastNews2?.news2Total !== undefined && lastNews2.news2Total >= 9,
...
test('quality.sparseSamplingFlag false for 3 visits', () => {
  ...
  assert.deepEqual(result.quality?.notes, ['visitCount: 3'])
})
...
test('quality.sparseSamplingFlag true for < 3 visits', () => {
  ...
  assert.deepEqual(result.quality?.notes, ['visitCount: 2'])
})
...
test('symptomsTimeline maps chief complaints in visit order and skips empty entries', () => {
  const visitsWithEmptyComplaint: VisitRecord[] = [
    mockVisits[0],
    { ...mockVisits[1], keluhan_utama: '   ' },
    mockVisits[2],
  ]

  const result = legacyIBToCtV1(mockAnalysis, visitsWithEmptyComplaint, 'patient-test-001')

  assert.equal(result.symptomsTimeline?.length, 2)
  assert.deepEqual(
    result.symptomsTimeline?.map((point) => ({
      id: point.id,
      observedAt: point.observedAt,
      source: point.source,
      symptom: point.symptom,
    })),
    [
      {
        id: 'sp-enc-001',
      ...
      },
      {
        ...
        source: 'imported',
        symptom: 'sesak napas berat',
      },
    ]
  )
})

test('symptomsTimeline stays undefined when all chief complaints are empty', () => {
  const silentVisits = mockVisits.map((visit) => ({ ...visit, keluhan_utama: ' ' }))
  const result = legacyIBToCtV1(mockAnalysis, silentVisits, 'patient-test-001')
  assert.equal(result.symptomsTimeline, undefined)
})

test('recommendations map deterministically to next-best fields without changing summary semantics', () => {
  const analysisWithRecommendations: TrajectoryAnalysis = {
    ...mockAnalysis,
    recommendations: [
      {
        category: 'action',
        priority: 'high',
        text: 'Ulangi pemeriksaan respirasi sekarang.',
      },
      {
        category: 'monitoring',
        priority: 'medium',
        text: 'Apakah kebutuhan oksigen meningkat sejak kunjungan terakhir?',
      },
    ],
  }

  const baselineResult = legacyIBToCtV1(mockAnalysis, mockVisits, 'patient-test-001')
  const result = legacyIBToCtV1(analysisWithRecommendations, mockVisits, 'patient-test-001')

  assert.equal(result.response.nextBestClinicalCheck, 'Ulangi pemeriksaan respirasi sekarang.')
  assert.equal(
    result.response.nextBestClinicalQuestion,
    'Apakah kebutuhan oksigen meningkat sejak kunjungan terakhir?'
  )
  assert.equal(result.response.summary, baselineResult.response.summary)
  assert.equal(result.response.severityBand, baselineResult.response.severityBand)
  assert.equal(result.response.momentum, baselineResult.response.momentum)
  assert.equal(
    result.response.recommendedMonitoringCadence,
    baselineResult.response.recommendedMonitoringCadence
  )
})

test('statement-only recommendations populate nextBestClinicalCheck without inventing question text', () => {
  const analysisWithStatementRecommendation: TrajectoryAnalysis = {
    ...mockAnalysis,
    recommendations: [
      {
        category: 'monitoring',
        priority: 'high',
        text: 'Monitor saturasi oksigen setiap 4 jam.',
      },
    ],
  }

  const result = legacyIBToCtV1(analysisWithStatementRecommendation, mockVisits, 'patient-test-001')

  assert.equal(result.response.nextBestClinicalCheck, 'Monitor saturasi oksigen setiap 4 jam.')
  assert.equal(result.response.nextBestClinicalQuestion, undefined)
})

test('baseline chronicDiseases preserves ICD code safe minimum from confirmed diagnoses', () => {
  const analysisWithChronicDiagnosis: TrajectoryAnalysis = {
    ...mockAnalysis,
    confirmed_chronic_diagnoses: [
      {
        icd_x: 'I10',
        ...
        confirmed_at: '2026-05-01T08:00:00.000Z',
      },
    ],
  }

  const result = legacyIBToCtV1(analysisWithChronicDiagnosis, mockVisits, 'patient-test-001')

  assert.deepEqual(result.baseline?.chronicDiseases, ['I10'])
})

test('blocked local-only fields remain unmapped from CT v1 adapter output', () => {
  const result = legacyIBToCtV1(mockAnalysis, mockVisits, 'patient-test-001')

  assert.equal(Object.prototype.hasOwnProperty.call(result, 'vitalTrends'), false)
  assert.equal(Object.prototype.hasOwnProperty.call(result, 'acute_attack_risk_24h'), false)
  assert.equal(Object.prototype.hasOwnProperty.call(result, 'time_to_critical_estimate'), false)
  assert.equal(Object.prototype.hasOwnProperty.call(result, 'overallRisk'), false)
  assert.equal(Object.prototype.hasOwnProperty.call(result.response, 'drivers'), false)
  assert.deepEqual(result.quality?.notes, ['visitCount: 3'])
})
...
test('T-48 derived point carries active_surge flag for CRP slope ≥ 37', () => {
  ...
  const t48Point = result.derivedTimeline?.find((d) => d.calculationBasis === 'standard_formula')
  assert.ok(
    t48Point?.flags?.includes('t48:active_surge'),
    `expected t48:active_surge, got ${JSON.stringify(t48Point?.flags)}`
  )
})
...
test('copdScale2: all NEWS2 derived points carry news2:scale2 flag', () => {
  ...
  const news2Points = result.derivedTimeline?.filter((d) => d.calculationBasis === 'official_score')
  assert.ok(
    news2Points?.every((d) => d.flags?.includes('news2:scale2')),
...
test('Phase F: T-49 derived point added with classification flag when gcsEvents supplied', () => {
  ...
  assert.ok(
    t49?.flags?.some((f) => f === 't49:active_decline'),
    `expected t49:active_decline, got ${JSON.stringify(t49?.flags)}`
  )
})
...
test('Phase F: backward compat — copdScale2 still works alongside gcsEvents', () => {
  const result = legacyIBToCtV1(
    ...
    undefined,
    undefined,
    { copdScale2: true, gcsEvents: mockGCSEvents }
  )
  const news2Points = result.derivedTimeline?.filter((d) => d.calculationBasis === 'official_score')
  assert.ok(
    news2Points?.every((d) => d.flags?.includes('news2:scale2')),
    'news2:scale2 flag must still be present'
  )
...
test('Phase F: T-49 observedAt matches last GCS event timestamp, not last visit timestamp', () => {
  ...
  assert.equal(
    t49?.observedAt,
    lastGcsObservedAt,
    `T-49 observedAt must be GCS lane timestamp, not ${lastVisitTimestamp}`
  )
})
