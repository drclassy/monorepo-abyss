---
id: "8c75f1af-b8e4-48f0-bbc2-c1d6087f25cb"
entity_type: "blueprint"
entity_id: "8c75f1af-b8e4-48f0-bbc2-c1d6087f25cb"
title: "Phase 1: Monorepo Foundation - Project Brief"
status: ""
priority: ""
updated_at: "2026-03-31T05:08:25.069086+00:00"
...
2. **Initialize local repository structure**
  ...
  git init the-abyss
 cd the-abyss
 git config user.name "Sentra Monorepo"
 git config user.email "monorepo@sentra.ai"
  ```
...
6. **Scaffold initial monorepo directories**
 ...
 mkdir -p apps/{healthcare,academic,incubator,internal,orchestrator}
 mkdir -p flows/{definitions,components,tests}
 mkdir -p tooling/{abyss-cli,generators}
 mkdir -p infrastructure/{docker,terraform,argocd,kubernetes}
 mkdir -p .agents
 mkdir -p docs/{adr,sentratorium/sessions,templates,dev}
  ```
...
4. **Create **`**docs/adr/0001-monorepo-strategy.md**` (first ADR)
 ...
 ## Decision
 We adopt a pnpm workspace monorepo (single Git repository) with Turborepo for build orchestration.
 ## Rationale
...
5. **Create **`**.agents/AGENTS.md**`** template**
 ...
 This document defines operational constraints and governance rules for AI agents operating within The Abyss.
...
6. **Create **`**docs/templates/HANDOFF.md**`** template**
  ```markdown
  # HANDOFF.md Template
 Use this template for all Phase 1 tasks and beyond.
 ---
 # Task: [Task Name]
 ## Diagnosis
...
1. **Create comprehensive root **`**README.md**`
 ...
 A unified monorepo foundation enabling human-AI collaborative development of healthcare applications.
 ## Quick Start
 ### Prerequisites
 ...
 ### Clone & Setup
 ```bash
 git clone https://github.com/sentra-ai/the-abyss.git
 cd the-abyss
 pnpm install        # Install all workspace dependencies
 pnpm turbo run build  # First full build with remote caching
  ```
  ### Development
  ...
  ### Governance
 All changes require a HANDOFF.md document with explicit approval (GO-Gate).
 See `.agents/AGENTS.md` and `docs/templates/HANDOFF.md`.
  ### Architectural Boundaries
 Apps are isolated by domain. Healthcare apps cannot import from non-healthcare packages without explicit architectural approval.
  ## Documentation
  - **[Developer Guide](docs/dev/getting-started.md)**: Onboarding and daily workflows
  - **[Architecture Decision Records](docs/adr/)**: Design decisions and rationale
  - **[Workspace Guide](docs/dev/workspace-setup.md)**: pnpm and Turbo configuration
  - **[Code Quality Standards](docs/dev/code-quality.md)**: Linting, formatting, testing
  ## Contributing
 See [CONTRIBUTING.md](CONTRIBUTING.md) for branch naming, PR process, and commit conventions.
  ## License
 Proprietary — Sentra Artificial Intelligence Inc.
 ```
2. **Create **`**CONTRIBUTING.md**` for development workflow
 ...
 Agent: [Your name or AI identifier]
 Phase: [Which phase, e.g., P1]
 Handoff: [Link to HANDOFF.md if applicable]
 ...
 Agent: DevOps-Lead
 Phase: P1
 Handoff: docs/handoffs/001-turborepo-setup.md
...
4. **Create **`**docs/dev/getting-started.md**` - New developer onboarding
 ...
 Welcome to Sentra's digital factory. This guide walks you through your first day of development.
 ...
 Watch the terminal as Turbo calculates the dependency graph and builds only affected packages.
 ...
 **Import convention**: `import { Component } from '@the-abyss/ui'`
 ...
 pnpm format  # Auto-fix formatting
  ...
  5. Push and create PR:
    ...
    Tests (Vitest)
    Architecture validation (Nx linting rules)
    GO-Gate verification (checks for approval in HANDOFF.md)
    ur PR merges only when all checks pass.
 Common Commands
    `bash
    Development
    pm dev              # Run all apps locally
    pm dev --filter=@app/healthcare  # Run one app
    Building
    pm build            # Build everything
    pm build:affected   # Build only changed packages
    Testing & Linting
    pm test             # Run all tests
    pm lint             # Check for errors
    pm format           # Auto-fix formatting
    Debugging
    pm turbo run build --graph=trace.html  # See build dependency graph
    `
 Asking for Help
...
5. **Create **`**docs/dev/workspace-setup.md**` - pnpm/Turbo reference
 ...
 pnpm allows multiple packages to share a single `node_modules` directory structure, dramatically reducing disk space and install time.
 ### Adding a New Package
  ...
  4. Your new package is instantly available as `@the-abyss/my-package` to other packages
    ...
    althcare:build: cache hit, replaying output
...
6. **Create **`**docs/dev/code-quality.md**` - Linting and testing standards
 ...
 All packages use TypeScript strict mode. This means:
 ...
 Auto-fix many issues: `pnpm lint -- --fix`
  ## Testing
 Minimum coverage: **80%**
 Write tests for:
  ...
  ## Prettier Formatting
 All code is auto-formatted before commit via Husky:
 Or format manually:
...
4. **Create **`**.github/pull_request_template.md**` - Standardized PR format
 ...
 Link to HANDOFF.md documenting this change (required for non-trivial changes).
 ## Testing
