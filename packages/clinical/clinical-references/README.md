...
`@the-abyss/clinical-references` adalah package sibling untuk reference-heavy
clinical assets yang mendukung SYMPHONY tanpa membuat core engine menjadi gemuk.
...
- `traffic-light` tetap hidup di SYMPHONY dan hanya akan mengonsumsi output
  ternormalisasi dari package ini pada fase berikutnya.
...
- Jangan commit dataset besar atau reference artifact pihak ketiga tanpa license
  review dan attribution review yang eksplisit.
...
- Phase 7c: SYMPHONY `traffic-light` mengonsumsi output ternormalisasi dari
  package ini.
- Phase 7d: loader/adapters reference resmi setelah licensing dan provenance
  dikunci.
