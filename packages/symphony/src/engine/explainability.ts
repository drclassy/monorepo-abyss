...
const ARBITER_REASON_NARRATIVE: Record<string, string> = {
  safety_critical_alert_present:
    'Alert kritikal terdeteksi — verifikasi keselamatan pasien.',
  native_must_not_miss_visible:
    'Diagnosis must-not-miss aktif — pertimbangkan workup kritis.',
  consciousness_compromised:
    'Tingkat kesadaran terganggu — tinjau jalur napas dan perfusi.',
  treatment_response_worsening:
    'Respons terapi memburuk — review eskalasi terapi.',
...
function describeReasons(reasons: readonly string[]): string {
  return reasons
    .map(key => ARBITER_REASON_NARRATIVE[key] ?? key)
    .join(' ')
}
