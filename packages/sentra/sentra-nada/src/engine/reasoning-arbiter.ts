...
import type { SymphonyPersonalBaseline, SymphonyTreatmentResponse } from './trajectory'
...
export function arbitrateSymphonyReasoning(
  ...
  const hasMustNotMiss = input.nativeHypotheses.some((item) => item.category === 'must_not_miss')
  ...
  const hasWorking = input.nativeHypotheses.some((item) => item.category === 'working')
  if (input.personalBaseline !== undefined && input.personalBaseline.visitCount < 2 && hasWorking) {
