...
describe('SYMPHONY pattern engine — field resolver', () => {
  ...
  it('resolves nested 2-deep field (derived.shockIndex)', () => {
    const pattern = makePattern({
      requiredCriteria: [
        { field: 'derived.shockIndex', op: 'lte', value: 1.0, label: 'SI <= 1.0' },
      ],
    })
...
describe('SYMPHONY pattern engine — criterion operators', () => {
  it('op:gte passes when field >= value', () => {
    ...
    expect(
      evaluateSymphonyPatterns(
        makeSnapshot({
          vitals: { sbp: 120, dbp: 80, hr: 80, rr: 15, temp: 36.5, spo2: 98, glucose: 100 },
        }),
        [pattern]
      )
    ).toHaveLength(0)
  ...
  it('op:lte passes when field <= value', () => {
    ...
    expect(
      evaluateSymphonyPatterns(
        makeSnapshot({
          vitals: { sbp: 120, dbp: 80, hr: 80, rr: 16, temp: 36.5, spo2: 94, glucose: 100 },
        }),
        [pattern]
      )
    ).toHaveLength(1)
    expect(
      evaluateSymphonyPatterns(
        makeSnapshot({
          vitals: { sbp: 120, dbp: 80, hr: 80, rr: 16, temp: 36.5, spo2: 95, glucose: 100 },
        }),
        [pattern]
      )
    ).toHaveLength(0)
  ...
  it('op:gt passes when field > value', () => {
    ...
    expect(
      evaluateSymphonyPatterns(
        makeSnapshot({
          vitals: { sbp: 120, dbp: 80, hr: 101, rr: 16, temp: 36.5, spo2: 98, glucose: 100 },
        }),
        [pattern]
      )
    ).toHaveLength(1)
    expect(
      evaluateSymphonyPatterns(
        makeSnapshot({
          vitals: { sbp: 120, dbp: 80, hr: 100, rr: 16, temp: 36.5, spo2: 98, glucose: 100 },
        }),
        [pattern]
      )
    ).toHaveLength(0)
  ...
  it('op:lt passes when field < value', () => {
    ...
    expect(
      evaluateSymphonyPatterns(
        makeSnapshot({
          vitals: { sbp: 89, dbp: 60, hr: 80, rr: 16, temp: 36.5, spo2: 98, glucose: 100 },
        }),
        [pattern]
      )
    ).toHaveLength(1)
    expect(
      evaluateSymphonyPatterns(
        makeSnapshot({
          vitals: { sbp: 90, dbp: 60, hr: 80, rr: 16, temp: 36.5, spo2: 98, glucose: 100 },
        }),
        [pattern]
      )
    ).toHaveLength(0)
  ...
  it('op:true passes when field === true', () => {
    const pattern = makePattern({
      requiredCriteria: [
        { field: 'history.knownHTN', op: 'true', value: true, label: 'Known HTN' },
      ],
    })
  ...
  it('op:in passes when field value is in comma-separated list', () => {
    const pattern = makePattern({
      requiredCriteria: [
        { field: 'derived.avpuLevel', op: 'in', value: 'V,P,U', label: 'AVPU impaired' },
      ],
    })
...
describe('SYMPHONY pattern engine — criteria logic', () => {
  it('ALL required criteria must pass (AND)', () => {
    ...
    const snapBoth = makeSnapshot({
      vitals: { sbp: 120, dbp: 80, hr: 110, rr: 24, temp: 36.5, spo2: 98, glucose: 100 },
    })
    const snapOne = makeSnapshot({
      vitals: { sbp: 120, dbp: 80, hr: 90, rr: 24, temp: 36.5, spo2: 98, glucose: 100 },
    })
...
describe('SYMPHONY pattern engine — requiresVitals guard', () => {
  it('skips pattern when required vital is 0 (not entered)', () => {
    ...
    const snapMissing = makeSnapshot({
      vitals: { sbp: 0, dbp: 80, hr: 80, rr: 16, temp: 36.5, spo2: 98, glucose: 100 },
    })
  ...
  it('evaluates pattern when all required vitals are present (> 0)', () => {
    ...
    const snapPresent = makeSnapshot({
      vitals: { sbp: 120, dbp: 80, hr: 80, rr: 16, temp: 36.5, spo2: 98, glucose: 100 },
    })
...
describe('SYMPHONY pattern engine — confidence', () => {
  ...
  it('scored boost: ratio 1.0 applies 1.0x multiplier', () => {
    ...
    const snap = makeSnapshot({
      vitals: { sbp: 120, dbp: 80, hr: 100, rr: 22, temp: 36.5, spo2: 98, glucose: 100 },
    })
    const result = evaluateSymphonyPatterns(snap, [pattern])
