...
Security and validation layer for the Abyss monorepo. Provides Express middleware for JWT (HS256-only) and API key authentication, plus a CI/CD GO-Gate validator that blocks pipelines until Chief approves session handoffs.
...
```typescript
import { authMiddleware, loadApiKeysFromEnv } from '@the-abyss/iskandar-gatekeeper'
...
Run as a CI step to block builds until all pending sessions have Chief GO approval:
